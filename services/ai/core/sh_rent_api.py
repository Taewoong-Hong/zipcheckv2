"""국토교통부 단독다가구 전월세 실거래가 자료 API 클라이언트 (리팩토링)."""
import logging
...
class SHRentAPIClient(RTMSBaseClient):
    """
    국토교통부 단독다가구 전월세 실거래가 자료 API 클라이언트.
    ...
    def __init__(self, api_key: str = None):
        ...
        logger.info("단독다가구 전월세 실거래가 API 클라이언트 초기화 완료")
    ...
    async def get_sh_rent(
        ...
        logger.info(f"단독다가구 전월세 조회: 지역={lawd_cd}, 계약년월={deal_ymd}")
        ...
        logger.info(f"단독다가구 전월세 조회 성공: {result['body']['totalCount']}건")
        return result
