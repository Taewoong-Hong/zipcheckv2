"""국토교통부 상업업무용 부동산 매매 실거래가 자료 API 클라이언트 (리팩토링)."""
import logging
...
class NrgTradeAPIClient(RTMSBaseClient):
    """
    ...
    async def get_nrg_trade(
        ...
        logger.info(f"상업업무용 매매 조회: 지역={lawd_cd}, 계약년월={deal_ymd}")
        ...
        logger.info(f"상업업무용 매매 조회 성공: {result['body']['totalCount']}건")
        return result
