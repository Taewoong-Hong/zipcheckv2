"""국토교통부 아파트 분양권 전매 실거래가 자료 API 클라이언트."""
import logging
...
class AptSilvTradeAPIClient:
    """
    국토교통부 아파트 분양권 전매 실거래가 자료 API 클라이언트.

    「부동산 거래신고 등에 관한 법률」에 따른 아파트 분양권 전매 실거래 신고 자료 제공
    ...
    API 문서: apis.data.go.kr/1613000/RTMSDataSvcSilvTrade
    """

    BASE_URL = "https://apis.data.go.kr/1613000/RTMSDataSvcSilvTrade/getRTMSDataSvcSilvTrade"
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        logger.info("아파트 분양권 전매 실거래가 API 클라이언트 초기화 완료")
    ...
    async def get_apt_silv_trade(
        ...
        logger.info(
            f"아파트 분양권 전매 조회: 지역={lawd_cd}, 계약년월={deal_ymd}, 페이지={page_no}"
        )
        ...
        try:
            ...
            logger.info(
                f"아파트 분양권 전매 API 응답: status={response.status_code}, "
            ...
            logger.info(f"아파트 분양권 전매 조회 성공: {len(item_data)}건 (총 {total_count}건)")
            ...
            logger.error(f"아파트 분양권 전매 조회 실패: {e}")
            raise

    def parse_trade_item(self, item: Dict[str, Any]) -> Dict[str, Any]:
        """
        아파트 분양권 전매 항목을 사용하기 쉬운 형태로 파싱.
