"""국토교통부 오피스텔 전월세 실거래가 자료 API 클라이언트 (리팩토링)."""
import logging
...
class OfficetelRentAPIClient(RTMSBaseClient):
    """
    ...
    async def get_officetel_rent(
        ...
        logger.info(f"오피스텔 전월세 조회: 지역={lawd_cd}, 계약년월={deal_ymd}")
        ...
        logger.info(f"오피스텔 전월세 조회 성공: {result['body']['totalCount']}건")
        return result
