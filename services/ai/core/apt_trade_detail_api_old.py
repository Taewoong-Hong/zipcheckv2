...
class AptTradeDetailAPIClient:
    """
    국토교통부 아파트 매매 실거래가 상세 자료 API 클라이언트.

    기존 아파트 실거래가 API보다 더 상세한 정보 제공:
    - 층정보 (floor)
    - 동정보 (aptDong) - 소유권 이전등기 완료 건만
    - 중개사 정보 (estateAgentSggNm)
    - 거래유형 (cdealType)
    - 매도자/매수자 구분 (slerGbn, buyerGbn)
    - 토지임대여부 (landLeaseholdGbn)

    API 문서: apis.data.go.kr/1613000/RTMSDataSvcAptTradeDev
    """
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        logger.info("아파트 매매 실거래가 상세 API 클라이언트 초기화 완료")
    ...
    async def get_apt_trade_detail(
        ...
        try:
            ...
            if not (2006 <= year <= 2100):
                raise ValueError(f"연도가 유효하지 않습니다: {year}")
            if not (1 <= month <= 12):
                raise ValueError(f"월이 유효하지 않습니다: {month}")
        except Exception as e:
            raise ValueError(f"계약년월 형식이 잘못되었습니다: {deal_ymd}") from e

        # 요청 파라미터 구성
        # 공식 문서 권장: serviceKey는 있는 그대로, 나머지 파라미터만 인코딩
        from urllib.parse import urlencode

        # serviceKey는 별도로 처리 (문서 원칙: URL Encode된 키를 그대로 사용)
        ...
        try:
            ...
            logger.info(
                f"아파트 실거래가 상세 API 응답: status={response.status_code}, "
            ...
            logger.info(f"아파트 실거래가 상세 조회 성공: {len(item_data)}건 (총 {total_count}건)")
            ...
            logger.error(f"아파트 실거래가 상세 조회 실패: {e}")
            raise

    async def get_apt_trades_by_period(
        self,
        lawd_cd: str,
        start_ym: str,
        end_ym: str,
        ...
        """
        기간별 아파트 매매 실거래가 상세 자료 조회.

        Args:
            lawd_cd: 지역코드 (5자리)
            start_ym: 시작년월 (YYYYMM)
            end_ym: 종료년월 (YYYYMM)
        ...
        Example:
            >>> # 2023년 1월~12월 강남구 실거래
            >>> trades = await client.get_apt_trades_by_period(
            ...     lawd_cd="11680",
            ...     start_ym="202301",
            ...     end_ym="202312"
            ... )
        ...
        while current_date <= end_date:
            ...
            try:
                result = await self.get_apt_trade_detail(
                    lawd_cd=lawd_cd,
                    deal_ymd=deal_ymd,
                    num_of_rows=1000,
                )
                ...
                logger.info(f"{deal_ymd}: {len(items)}건 조회")

            except Exception as e:
                logger.error(f"{deal_ymd} 조회 실패: {e}")

            current_date += relativedelta(months=1)

        logger.info(f"기간별 조회 완료: {start_ym}~{end_ym}, 총 {len(all_trades)}건")
        return all_trades

    def parse_trade_item(self, item: Dict[str, Any]) -> Dict[str, Any]:
        """
        실거래 항목을 사용하기 쉬운 형태로 파싱.
        ...
        return {
            ...
            "지역코드_읍면동": item.get("umdCd"),
            "읍면동명": item.get("umdNm"),
            "지번": item.get("jibun"),
            "도로명": item.get("roadNm"),
            "도로명번호본번": item.get("roadNmBonbun"),
            "도로명번호부번": item.get("roadNmBubun"),

            # 아파트 정보
            "아파트명": item.get("aptNm"),
            "아파트동": item.get("aptDong"),  # 소유권이전등기 완료 건만
            "전용면적": float(item.get("excluUseAr", 0)),
            "층": item.get("floor"),
            "건축년도": item.get("buildYear"),
            "아파트일련번호": item.get("aptSeq"),
            ...
            # 기타
            "토지임대여부": item.get("landLeaseholdGbn"),
