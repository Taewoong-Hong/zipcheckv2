...
class LandTradeAPIClient:
    """
    국토교통부 토지 매매 실거래가 자료 API 클라이언트.

    「부동산 거래신고 등에 관한 법률」에 따른 토지 매매 실거래 신고 자료 제공
    ...
    API 문서: apis.data.go.kr/1613000/RTMSDataSvcLandTrade
    """
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        logger.info("토지 매매 실거래가 API 클라이언트 초기화 완료")
    ...
    async def get_land_trade(
        ...
        logger.info(
            f"토지 매매 조회: 지역={lawd_cd}, 계약년월={deal_ymd}"
        )
        ...
        try:
            ...
            logger.info(
                f"토지 매매 API 응답: status={response.status_code}, "
            ...
            logger.info(f"토지 매매 조회 성공: {len(item_data)}건 (총 {total_count}건)")
    ...
    def parse_trade_item(self, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            # 토지 정보
            "토지면적": float(item.get("landArea", 0)),
            "용도지역": item.get("landUse"),
            "지목": item.get("landClass"),

            # 거래 정보
            "거래일자": deal_date,
            "거래금액_원": deal_amount,
            "거래금액_만원": deal_amount // 10000,
            "거래유형": item.get("dealType"),
