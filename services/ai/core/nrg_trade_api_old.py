"""국토교통부 상업업무용 부동산 매매 실거래가 자료 API 클라이언트."""
import logging
...
class NrgTradeAPIClient:
    """
    국토교통부 상업업무용 부동산 매매 실거래가 자료 API 클라이언트.

    「부동산 거래신고 등에 관한 법률」에 따른 상업업무용 부동산 매매 실거래 신고 자료 제공
    ...
    API 문서: apis.data.go.kr/1613000/RTMSDataSvcNrgTrade
    """
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        logger.info("상업업무용 부동산 매매 실거래가 API 클라이언트 초기화 완료")
    ...
    async def get_nrg_trade(
        ...
        logger.info(
            f"상업업무용 부동산 매매 조회: 지역={lawd_cd}, 계약년월={deal_ymd}"
        )
        ...
        try:
            ...
            logger.info(
                f"상업업무용 부동산 매매 API 응답: status={response.status_code}, "
            ...
            logger.info(f"상업업무용 부동산 매매 조회 성공: {len(item_data)}건 (총 {total_count}건)")
            ...
            logger.error(f"상업업무용 부동산 매매 조회 실패: {e}")
            raise

    def parse_trade_item(self, item: Dict[str, Any]) -> Dict[str, Any]:
        """
        상업업무용 부동산 매매 항목을 사용하기 쉬운 형태로 파싱.
