"""국토교통부 공장 및 창고 등 부동산 매매 실거래가 자료 API 클라이언트."""
import logging
...
class InduTradeAPIClient:
    """
    국토교통부 공장 및 창고 등 부동산 매매 실거래가 자료 API 클라이언트.

    「부동산 거래신고 등에 관한 법률」에 따른 공장 및 창고 등 부동산 매매 실거래 신고 자료 제공

    특징:
    - 공장 및 창고 등 부동산 매매 실거래 정보
    - 건축물 유형 및 용도 정보
    - 토지 용도 및 면적 정보
    - 지분 거래 구분
    - 매도자/매수자 구분
    - 중개사 정보 (소재지)
    - 개인정보 보호를 위해 일반건축물의 지번 정보는 일부만 제공

    API 문서: apis.data.go.kr/1613000/RTMSDataSvcInduTrade
    """
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        logger.info("공장 및 창고 등 부동산 매매 실거래가 API 클라이언트 초기화 완료")
    ...
    async def get_indu_trade(
        ...
        logger.info(
            f"공장 및 창고 등 부동산 매매 조회: 지역={lawd_cd}, 계약년월={deal_ymd}"
        )
        ...
        try:
            ...
            logger.info(
                f"공장 및 창고 등 부동산 매매 API 응답: status={response.status_code}, "
            ...
            logger.info(f"공장 및 창고 등 부동산 매매 조회 성공: {len(item_data)}건 (총 {total_count}건)")
            ...
            logger.error(f"공장 및 창고 등 부동산 매매 조회 실패: {e}")
            raise

    def parse_trade_item(self, item: Dict[str, Any]) -> Dict[str, Any]:
        """
        공장 및 창고 등 부동산 매매 항목을 사용하기 쉬운 형태로 파싱.
        ...
        return {
            ...
            # 건축물 정보
            "건축물유형": item.get("buildingType"),
            "건축물용도": item.get("buildingUse"),
            "건축면적": float(item.get("buildingAr", 0)),
            "층": item.get("floor"),
            "건축년도": item.get("buildYear"),

            # 토지 정보
            "토지용도": item.get("landUse"),
            "대지면적": float(item.get("plottageAr", 0)),
            ...
            "지분거래구분": item.get("shareDealingType"),
