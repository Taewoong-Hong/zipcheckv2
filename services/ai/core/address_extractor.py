...
def extract_address_from_text(text: str) -> AddressExtractionResult:
    ...
    patterns = [
        ...
        # Pattern 2: 경기도/도 + 도로명 (구/동 생략된 형태)
        r"((?:경기도|강원도|충청북도|충청남도|전라북도|전라남도|경상북도|경상남도)\s*[가-힣\d\-]+(?:로|길)\s*\d+(?:\-\d+)?)",

        # Pattern 3: 시/도 + 도로명 (간단한 형태)
        r"((?:서울|부산|대구|인천|광주|대전|울산|세종|경기|강원|충북|충남|전북|전남|경북|경남|제주)\s*[가-힣\d\-]+(?:로|길)\s*\d+(?:\-\d+)?)",
