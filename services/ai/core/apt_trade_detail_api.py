"""국토교통부 아파트 매매 실거래가 상세 자료 API 클라이언트 (리팩토링)."""
import logging
...
class AptTradeDetailAPIClient(RTMSBaseClient):
    """
    ...
    async def get_apt_trade_detail(
        ...
        logger.info(
            f"아파트 실거래가 상세 조회 성공: {result['body']['totalCount']}건"
        )
