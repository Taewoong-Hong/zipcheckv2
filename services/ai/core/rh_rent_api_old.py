"""국토교통부 연립다세대 전월세 실거래가 자료 API 클라이언트."""
import logging
...
class RHRentAPIClient:
    """
    국토교통부 연립다세대 전월세 실거래가 자료 API 클라이언트.

    「부동산 거래신고 등에 관한 법률」 및 「주택임대차보호법」에 따른
    연립다세대 주택 전월세 실거래 및 확정일자 자료 제공
    ...
    API 문서: apis.data.go.kr/1613000/RTMSDataSvcRHRent
    """
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        logger.info("연립다세대 전월세 실거래가 API 클라이언트 초기화 완료")
    ...
    async def get_rh_rent(
        ...
        logger.info(
            f"연립다세대 전월세 조회: 지역={lawd_cd}, 계약년월={deal_ymd}"
        )
        ...
        try:
            ...
            logger.info(
                f"연립다세대 전월세 API 응답: status={response.status_code}, "
            ...
            logger.info(f"연립다세대 전월세 조회 성공: {len(item_data)}건 (총 {total_count}건)")
