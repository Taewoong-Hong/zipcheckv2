...
class AptRentAPIClient:
    """
    국토교통부 아파트 전월세 실거래가 자료 API 클라이언트.

    「부동산 거래신고 등에 관한 법률」 및 「주택임대차보호법」에 따른
    아파트 전월세 실거래 및 확정일자 자료 제공
    ...
    - 갱신요구권 사용여부
    - 종전 보증금/월세 정보
    - 개인정보 보호를 위해 동/호 정보는 제공되지 않음

    API 문서: apis.data.go.kr/1613000/RTMSDataSvcAptRent
    """
    ...
    async def get_apt_rent(
        ...
        try:
            ...
            logger.info(
                f"아파트 전월세 API 응답: status={response.status_code}, "
            ...
            logger.info(f"아파트 전월세 조회 성공: {len(item_data)}건 (총 {total_count}건)")
    ...
    async def get_apt_rents_by_period(
        self,
        lawd_cd: str,
        start_ym: str,
        end_ym: str,
        ...
        Example:
            >>> # 2023년 1월~12월 강남구 전월세
            >>> rents = await client.get_apt_rents_by_period(
            ...     lawd_cd="11680",
            ...     start_ym="202301",
            ...     end_ym="202312"
            ... )
        ...
        while current_date <= end_date:
            ...
            try:
                result = await self.get_apt_rent(
                    lawd_cd=lawd_cd,
                    deal_ymd=deal_ymd,
                    num_of_rows=1000,
                )
        ...
        logger.info(f"기간별 조회 완료: {start_ym}~{end_ym}, 총 {len(all_rents)}건")
        return all_rents
