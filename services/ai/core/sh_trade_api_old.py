"""국토교통부 단독/다가구 매매 실거래가 자료 API 클라이언트."""
import logging
...
class SHTradeAPIClient:
    """
    국토교통부 단독/다가구 매매 실거래가 자료 API 클라이언트.

    「부동산 거래신고 등에 관한 법률」에 따른 단독/다가구 주택 매매 실거래 신고 자료 제공
    ...
    API 문서: apis.data.go.kr/1613000/RTMSDataSvcSHTrade
    """
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        logger.info("단독/다가구 매매 실거래가 API 클라이언트 초기화 완료")
    ...
    async def get_sh_trade(
        ...
        logger.info(
            f"단독/다가구 매매 조회: 지역={lawd_cd}, 계약년월={deal_ymd}"
        )
        ...
        try:
            ...
            logger.info(
                f"단독/다가구 매매 API 응답: status={response.status_code}, "
            ...
            logger.info(f"단독/다가구 매매 조회 성공: {len(item_data)}건 (총 {total_count}건)")
            ...
            logger.error(f"단독/다가구 매매 조회 실패: {e}")
            raise

    def parse_trade_item(self, item: Dict[str, Any]) -> Dict[str, Any]:
        """
        단독/다가구 매매 항목을 사용하기 쉬운 형태로 파싱.
