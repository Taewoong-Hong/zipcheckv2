"""국토교통부 공장·창고 등 부동산 매매 실거래가 자료 API 클라이언트 (리팩토링)."""
import logging
...
class InduTradeAPIClient(RTMSBaseClient):
    """
    국토교통부 공장·창고 등 부동산 매매 실거래가 자료 API 클라이언트.
    ...
    def __init__(self, api_key: str = None):
        ...
        logger.info("공장·창고 등 부동산 매매 실거래가 API 클라이언트 초기화 완료")
    ...
    async def get_indu_trade(
        ...
        logger.info(f"공장·창고 매매 조회: 지역={lawd_cd}, 계약년월={deal_ymd}")
        ...
        logger.info(f"공장·창고 매매 조회 성공: {result['body']['totalCount']}건")
        return result
